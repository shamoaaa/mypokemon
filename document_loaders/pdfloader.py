...
class UnstructuredLightPipeline(object):
    def __init__(self, file_path):
        # 构造函数：初始化UnstructuredProcessor实例
        self.file_path = file_path

    def extract_data(self, strategy, model_name, options, local=True, debug=False):
        """
        从指定的文件中提取数据。
        ...
        elements = self.invoke_pipeline_step(
            lambda: self.process_file(strategy, model_name),
        ...
        if debug:
            ...
            new_file_path = self.change_file_extension(new_extension)
    ...
    def process_file(self, strategy, model_name):
        """
    ...
    def change_file_extension(self, new_extension, suffix=None):
    ...
    def load(self):
        # 提取PDF中的表格数据
        content, table_content = self.extract_data(
        'hi_res',       # 
